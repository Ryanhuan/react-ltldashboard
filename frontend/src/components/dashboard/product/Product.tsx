...
import { checkEmpty } from '@/utility'
...
const initialState = {
    ...
    insertData: {
        name: '', kind: '', size: '', inventory: 0, status: '',
...
export class Product extends Component<any, StateType> {
    constructor(props) {
        super(props);
    ...
    // 計算總計&利潤
    cntPriceAndProfit() {
        if (Number(this.state.insertData.product_price) >= 0 || this.state.insertData.product_price == undefined) {
            ...
            this.setState((prev) => (
                {
                    insertData: {
                        ...prev.insertData,
                        product_items_total_price: tmpTotalPrice,
                        product_profit: Math.round((Number(prev.insertData.product_price) - tmpTotalPrice) * 100) / 100
                    }
                }));
        }
    ...
    async handleInsertDataChange(event) {
        ...
        switch (event.target.name) {
            ...
            case 'product_price':
                if (Number(this.state.insertData.product_price) >= 0 || this.state.insertData.product_price == undefined) {
                    _insertData.product_profit = Math.round((Number(_insertData.product_price) - _insertData.product_items_total_price) * 100) / 100
    ...
    async handleSubmitInsertData(event) {
        ...
        if (this.state.insertData.catena_belong === '' || this.state.insertData.single_belong === '') {
            ...
            let _res = await postData('/api/product/handleProductData', data);
            if (_res.ack === 'OK') {
                if (this.props.type === 'edit') {
                    customToastTopEnd.fire('Success!', 'Edit product successfully!', 'success');
    ...
    handleDeleteItem(fileId) { this.setState((prev) => ({ fileList: prev.fileList.filter((item: { id: string }) => item.id !== fileId) })); };
    ...
    render() {
        ...
        const maListTmp = this.state.maList.map((item, index) => (
            <Row key={index}>
                <Col xs={1} md={1} className={styles.maListItemCol}>
                    <Button variant="contained" startIcon={<ClearIcon />} themeColor="error" onClick={() => this.deleteMaListItem(index)}
                        style={{
                            paddingLeft: '7px',
                            borderRadius: 50,
                            minWidth: '1px',
                ...
                <Col xs={2} md={2} className={styles.maListItemCol}>
                    單價 :{item.price_per}
                </Col>
                <Col xs={2} md={2} className={styles.maListItemCol}>
                    小計 :{item.itemPrice | 0}
            ...
            <div className={styles.product}>
                ...
                <div className={styles.wrapper}>
                    ...
                    <div className={styles.bodyLeft}>
                        ...
                        {/* 基本資料 */}
                        <div className={clsx(styles.item, 'neumorphismFlat')}>
                                        ...
                                        <Row>
                                            <Col xs={12} md={6}>
                                                <FloatingLabel controlId="floatingInputInventory" label="庫存" className="mb-2 ">
                                                    <Form.Control type="number" placeholder="庫存" name='inventory' value={this.state.insertData.inventory} onChange={this.handleInsertDataChange} />
                                                </FloatingLabel>
                                            </Col>
                                        </Row> : null
                                    }
                        ...
                        {/* Hashtags */}
                        {/* <div className={clsx(styles.item,'neumorphismFlat')}>
                                <div className={styles.itemTitle}> 新增 Hashtags </div>
                                <div className={styles.itemWrapper}>
                                    <Container>
                                        <Row>
                                            <Col xs={12} md={12}>
                        ...
                        {/* PIC WALL */}
                        <div className={clsx(styles.item, 'neumorphismFlat')}>
                            <div className={styles.itemTitle}> 商品照上傳 </div>
                            <div className={styles.itemWrapper}>
                                <Container>
                                    <Row>
                                        <Col xs={12} md={12}>
                    ...
                    <div className={styles.bodyRight}>
                        ...
                        <div className={clsx(styles.item, 'neumorphismFlat')}>
                            <div className={styles.itemTitle}> </div>
                            <div className={styles.itemWrapper} style={{ minWidth: '600px' }}>
                                <Container>
                                    <Row>
                                        <Col xs={12} md={4}>
                        ...
                        {/* <ModalMaList /> */}
                        <ModalMaList isShow={this.state.modal.isShow} onHide={this.modalOnHide} data={this.state.maList}
                            submitForm={(e, data) => { this.submitForm(e, data) }} />

                        <div className={clsx(styles.sendBtn, 'neumorphismFlat')}>
...
type StateType = {
    ...
    modal?: { isShow: boolean },
    insertData?: I_insertData,
    hashtags?: {}[],
    fileList?: {}[], // upload
    maList?: I_maList[], // 使用材料清單
}

interface I_selectOption {
    product_kind: { value: string, label: string }[],
    product_status: { value: string, label: string }[],
    product_catena: { value: string, label: string }[],
    product_single: { value: string, label: string }[],
}
...
interface I_insertData {
    ...
    inventory: number,
    status: string,
    catena_belong: string,
    single_belong: string,
    catena_intro: string,
    product_intro: string,
    other_intro: string,
    scheduled_date: string,
    limit_date_beg: string,
    limit_date_end: string,
    product_items_total_price: number,
